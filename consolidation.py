...
st.markdown("Upload your Excel file (with 'Compiled Data' and supplier sheets) to get consolidated CSV.")
...
def find_description_column(df):
    patterns = [r'kinshasa.*description', r'product.*description', r'erp.*description', r'item name', r'description']
    for col in df.columns:
    ...
    with st.spinner("Processing file..."):
        try:
            ...
            if result_df is not None:
                ...
                st.download_button("📥 Download Consolidated CSV", csv_data, "final_stock_with_suppliers.csv", "text/csv")
                
                # Verification summary
                total_items = len(result_df)
                items_with_stock = result_df[result_df["PHYSICAL_STOCK"].notna() | result_df["PENDING_ORDERS"].notna() | result_df["IN_TRANSIT_TOTAL"].notna()]
                st.subheader("Verification Report")
                st.write(f"Total items: {total_items}")
                st.write(f"Items with stock data: {len(items_with_stock)}")
